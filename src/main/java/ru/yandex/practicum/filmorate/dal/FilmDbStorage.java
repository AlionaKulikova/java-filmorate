    ...
    public Film saveFilm(Film film) {
        ...
        if (!mpaExists) {
            log.error("Ошибка валидации: MPA не может быть пустым или иметь null id, или не существует в" +
                    " базе данных");
            throw new ConditionsNotMetException("MPA не может быть пустым, иметь null id или не существовать в" +
                    " базе данных");
        }
        ...
        if (film.getGenres() != null && !film.getGenres().isEmpty()) {
            ...
            for (Genre genre : filmGenres) {
                ...
                if (!genreExists) {
                    log.error("Ошибка валидации. Жанр с id = {} не существует в базе данных", genre.getId());
                    throw new ConditionsNotMetException("Жанр с id " + genre.getId() + " не существует в базе данных");
                }
            }
